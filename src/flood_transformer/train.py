...
def _build_candidate_edges(
    ...
    for s, t, sc in candidates:
        ...
        seen.add((s, t))
        dedup.append((s, t, sc))
        if top_k > 0 and len(dedup) >= max(1, top_k):
            break
    return dedup
...
def run_full_experiment(config: ExperimentConfig) -> Dict[str, str]:
    ...
    if len(circuit.get("active_heads", [])) > 0:
        ...
        l0_targets.extend([f"L0.H{i}" for i in np.where(h0 > 0)[0].tolist()])
    if len(circuit.get("active_neurons", [])) > 0:
        n0 = np.asarray(circuit["active_neurons"][0].cpu())
        l0_targets.extend([f"L0.N{i}" for i in np.where(n0 > 0)[0].tolist()])
    if config.in_node_bridge_max_nodes > 0:
        l0_targets = l0_targets[: config.in_node_bridge_max_nodes]

    in_node_bridge = in_node_interaction_test(
        model,
        data.test_loader,
        device,
        feature_names=data.feature_names,
        target_node_ids=l0_targets,
        mode="replace_mean",
        threshold=config.circuit_threshold,
    )
    ...
    in_node_bridge_json = os.path.join(config.output_dir, "in_node_interactions.json")
    with open(in_node_bridge_json, "w", encoding="utf-8") as f:
        json.dump(in_node_bridge, f, ensure_ascii=False, indent=2)
    in_node_bridge_csv = os.path.join(config.output_dir, "in_node_interactions_ranking.csv")
    pd.DataFrame(in_node_bridge.get("records", [])).to_csv(in_node_bridge_csv, index=False)

    # Unified artifact for reproducible downstream loading.
    real_bundle_json = os.path.join(config.output_dir, "real_importance_bundle.json")
    with open(real_bundle_json, "w", encoding="utf-8") as f:
        json.dump(
            {
                "edge_importance": edge_importance,
                "in_node_interactions": in_node_bridge,
            },
            f,
            ensure_ascii=False,
            indent=2,
        ...
        json.dump(
            {
                ...
                "outputs": {
                    ...
                    "in_node_interactions": in_node_bridge_json,
                    "in_node_interactions_ranking": in_node_bridge_csv,
                    "real_importance_bundle": real_bundle_json,
