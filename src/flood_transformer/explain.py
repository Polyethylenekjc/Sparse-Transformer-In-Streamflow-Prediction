...
def in_node_interaction_test(
    ...
    """Compute real bridge score between input feature i and internal node j.

    For each (i, j):
      I(i,j) = MSE(do(i),ablate(j)) - MSE(do(i)) - MSE(ablate(j)) + MSE(base)
    """
    model.eval()

    valid_nodes = [nid for nid in target_node_ids if _parse_node(nid) is not None]
    n_feat = len(feature_names)

    base_heads, base_neurons = _default_binary_overrides(model, device, threshold=threshold)
    base_mse = _eval_mse(model, data_loader, device, head_overrides=base_heads, neuron_overrides=base_neurons)

    feature_mse: Dict[int, float] = {}
    for i in range(n_feat):
        p, y = _collect_preds_feature_interventions(
            model,
            data_loader,
        ...
        feature_mse[i] = float(np.mean((p - y) ** 2))

    def _clone_overrides():
        return (
            {k: v.clone() for k, v in base_heads.items()},
            {k: v.clone() for k, v in base_neurons.items()},
        )

    node_mse: Dict[str, float] = {}
    for nid in valid_nodes:
        h, n = _clone_overrides()
        if not ablate_node_id(nid, h, n):
            continue
        node_mse[nid] = _eval_mse(model, data_loader, device, head_overrides=h, neuron_overrides=n)
    ...
    for i, feat in enumerate(feature_names):
        ...
        for nid in valid_nodes:
            ...
            for x, y in data_loader:
                ...
                series = x_mod[:, :, i]
                if mode == "replace_mean":
                    x_mod[:, :, i] = series.mean(dim=1, keepdim=True)
                elif mode == "shuffle_keep_total":
                    bsz, seq = series.shape
                    perm = torch.stack([torch.randperm(seq, device=x.device) for _ in range(bsz)], dim=0)
                    x_mod[:, :, i] = series.gather(1, perm)
                else:
                    raise ValueError(f"unknown intervention mode: {mode}")

                out = model(
                    x_mod,
                    head_mask_overrides=h,
                    neuron_mask_overrides=n,
                    return_intermediates=False,
                )
                preds.append(out.pred.detach().cpu().numpy())
                ys.append(y.detach().cpu().numpy())

            mse_ij = float(np.mean((np.concatenate(preds) - np.concatenate(ys)) ** 2))
            mse_j = float(node_mse[nid])
            interaction = float(mse_ij - mse_i - mse_j + base_mse)
            ...
            records.append(
                {
                    ...
                    "abs_interaction": float(abs(interaction)),
                }
            )

    records.sort(key=lambda d: float(d["abs_interaction"]), reverse=True)
    return {
        "mode": mode,
        "baseline_mse": float(base_mse),
        "feature_names": feature_names,
        "target_node_ids": valid_nodes,
        "records": records,
    }
