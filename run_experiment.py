...
def _resolve_station_id(station_id: str | None, forcing_dir: str, streamflow_dir: str) -> str:
    ...
    raise FileNotFoundError("No available station found. Please check forcing/streamflow directories")


def build_args():
    p = argparse.ArgumentParser(description="Explainable Sparse Transformer for Flood Prediction")
    p.add_argument("--station_id", type=str, default=None, help="Station ID, e.g., 01013500")
    p.add_argument("--forcing_dir", type=str, default="data/Forcing", help="Meteorological forcing data directory")
    p.add_argument("--streamflow_dir", type=str, default="data/Streamflow", help="Streamflow data directory")
    p.add_argument("--task", type=str, default="regression", choices=["regression", "classification"])
    p.add_argument("--nse_threshold", type=float, default=0.5, help="NSE pass threshold")
    ...
    p.add_argument("--lambda_mask_l1", type=float, default=1e-4, help="L1 regularization strength for head/neuron masks")
    p.add_argument("--lambda_input_mask_l1", type=float, default=1e-5, help="L1 regularization strength for input-node masks")
    p.add_argument("--circuit_threshold", type=float, default=0.49, help="Circuit pruning threshold")
    p.add_argument("--input_threshold", type=float, default=0.5, help="Input-node pruning threshold")
    p.add_argument("--edge_ablation_topk", type=int, default=0)
    p.add_argument("--in_node_bridge_max_nodes", type=int, default=32, help="Maximum number of target nodes evaluated for IN->node interaction; <=0 means no truncation")
    ...
    p.add_argument("--no_rich", action="store_true", help="Disable rich progress bars")
    p.add_argument("--log_every", type=int, default=20, help="Log debug information every N steps")
    p.add_argument("--output_dir", type=str, default="outputs")
    p.add_argument(
        "--include_q_input",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Whether to include historical streamflow Q as an input feature (enabled by default)",
    )
    return p.parse_args()
